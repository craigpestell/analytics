...
class Analytics {
  ...
  addEventListeners(analyticsDataMap) {
    // const getAnalyticsDataForElement = (dataMap) => (Promise.all(dataMap.map((p) => (resolvedDataMap(p)))))
    const getAnalyticsDataForElement = (dataMap) => {
      console.log('dataMap:', dataMap);
      // return Promise.props(dataMap)
      let combinedDataMap = {};
      Object.entries(dataMap).map((entry) => {
        const entity = entry[0];
        combinedDataMap[entity] = analyticsDataMap(dataMap[entity]) || this.defaultAnalyticsDataMap(dataMap[entity])
      });

      return Promise.props(combinedDataMap)
    }
